...
namespace Concordia
{
    ...
    public class DiscordClient
    {
        ...
        static string UserAgentString = $"DiscordBot (http://github.com/cuken/Concordia {typeof(DiscordClient).Assembly.GetName().Version.ToString()})";
        ...
        private void GetChannelsList(JObject m)
        {
            ...
            foreach(var j in m["d"]["guilds"])
            {
                ...
                foreach(var u in j["roles"])
                {
                    ...
                    {
                        color = new Concordia.Color(u["color"].ToObject<int>().ToString("x")),
        ...
        private void GuildUpdateEvents(JObject message)
        {
            ...
            if (!message["d"]["roles"].IsNullOrEmpty())
            {
                foreach (var roll in message["d"]["roles"])
                {
                    DiscordRole t = new DiscordRole
                    {
                        color = new Concordia.Color(roll["color"].ToObject<int>().ToString("x")),
